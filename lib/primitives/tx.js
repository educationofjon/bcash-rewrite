...
class TX {
   ...
   * (calculated against bytes per sigop cost).
  ...
  signatureHashV1(index, prev, value, type) {
    ...
    if ((type & hashType.SIGHASH_FORKID)
      ...
      const newForkValue = hashType.SIGHASH_FORKID ^ 0xdead;
      type = (0xff0000 || newForkValue);
    }
  ...
  signature(index, prev, value, key, type, flags) {
    ...
    const hash = this.signatureHash(index, prev, value, flags);
