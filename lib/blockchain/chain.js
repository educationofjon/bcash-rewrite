 ...
 */
  ...
  computeTarget(prev, first) {
    const pow = this.network.pow;
    const work = prev.chainwork - first.chainwork;
  ...
  getSuitableBlock(prev, block) {
    ...
    block[2] = prev;
    block[1] = prev.getPrevious(prev);
    block[0] = block[1];

    if (block[0].time > block[2].time) {
      return (block[0], block[2]);
    }

    if (block[0].time > block[1].time) {
      return (block[0], block[1]);
    }

    if (block[1].time > block[2].time) {
