...
class Chain extends AsyncEmitter {
  ...
  async verify(block, prev, flags) {
    ...
    if (block.getBaseSize() > consensus.MAX_BLOCK_SIZE) {
      throw new VerifyError(block,
        'invalid',
        'bad-blk-size',
        100);
  ...
  /**
   * Check all deployments on a chain, ranging from p2sh to monolith activation.
   * @param {Number} time
   * @param {ChainEntry} prev
   * @returns {Promise} - Returns {@link DeploymentState}.
   */
  ...
  async getDeployments(time, prev) {
    ...
    if (time >= this.network.block.monolithActivation)
  ...
  async getTarget(time, prev) {
    ...
    if (await this.isDAAEnabled(prev)) {
      // Do not retarget
      if (pow.targetReset) {
 ...
 * will fail if used.
 * @param {ChainEntru} prev - Previous Chain entry.
 * @returns {Promise}
 */
...
class DeploymentState {
  ...
  /**
   * Test whether the new Monolith Activation is enabled.
   * @returns {Boolean}
   */
