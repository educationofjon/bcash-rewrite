...
class Chain extends AsyncEmitter {
  ...
  async getDeployments(time, prev) {
    ...
    if (await this.isUAHFEnabled(prev)) {
      state.flags |= Script.flags.VERIFY_STRICTENC;
      state.flags |= Script.flags.VERIFY_SIGHASH_FORKID;
    }
 ...
 async cashRetarget(time, prev) {
   ...
   const height = prev.height + 1;
   assert(height >= pow.retargetInterval);


   const heightFirst = height - 144;
   const last = prev.getAncestor(heightFirst);

   const first = await this.getAncestor(heightFirst);

   const nextTarget = await this.computeTarget(first);

   if (nextTarget > pow.limit)
     return pow.bits;
 ...
 */
  ...
  computeTarget(prev, first) {
    ...
    const chainwork = prev.chainwork - first.chainwork;
    chainwork.imuln(pow.targetSpacing);

    let actualTimespan = prev.time - first.time;

    if (actualTimespan > 288 * pow.targetSpacing) {
      actualTimespan = 288* pow.targetSpacing;
    } else if (actualTimespan < 72 * pow.targetSpacing) {
      actualTimespan = 72 * pow.targetSpacing;
    }

    chainwork.idivn(actualTimespan);

    /*
     * We need to compute T = (2^256 / Chainwork) - 1
     * but 2^256 doesn't fit in 256 bits.
     * By Expression 1 as a valid Chainwork entry,
     * we'll be able to get (2^256 - Chainwork) / Chainwork,
     * therefore we'll be able to compute 2^256 - Chainwork as
     * the complement of the Chainwork.
     */

    return -chainwork / chainwork;
  }

  getSuitableBlock(prev, block) {
    const pow = this.network.pow;
    const height = prev.height + 1;

    const previous = this.getPrevious(prev);

    // In order to avoid a block with a
    // very skewed timestamp having too
    // much influence. We must select the
    // median of the top 3 blocks as an
    // starting point for our range.

    // pindex;
    const block2 = prev;
    // pindex->> pprev;
    const block1 = previous;
    // block1->>pprev;
    const block0 = this.getEntryByHeight(block1.height - 1);
  ...
  async isDAAEnabled(prev) {
    ...
    if (height >= this.network.block.daaHeight) {
      state.flags |= Script.flags.VERIFY_LOW_S;
      state.flags |= Script.flags.VERIFY_NULLFAILL;
    }
    return state;
  }
