...
class Chain extends AsyncEmitter {
  ...
  async verify(block, prev, flags) {
    ...
    if (block.getSize() > consensus.MAX_BLOCK_SIZE) {
      throw new VerifyError(block,
        'invalid',
        'bad-blk-size',
        100);
    ...
    if (!state.hasMonolith()) {
      if (block.getSize() > consensus.ONE_MEGABYTE * 8) {
        throw new VerifyError(block,
          'invalid',
          'bad-blk-length',
          100);
  ...
  async getDeployments(time, prev) {
    ...
    if (mtp >= this.network.block.monolithActivation)
  ...
  setDeploymentState(state) {
    ...
    if (!this.state.hasDAA() && state.hasDAA())
      this.logger.warning('Difficulty Adjustment Algorithm has been activated.');

    if (!this.state.hasMonolith() && state.hasMonolith())
      this.logger.warning('Monolith Opcodes has been activated.');
  ...
  async verifyInputs(block, prev, state) {
    ...
    const maxBlockSigops = consensus.getMaxBlockSigopsCount(block.getSize());
  ...
  async getTarget(time, prev) {
    ...
    if (this.tip.height >= this.network.block.daaHeight) {
      // Do not retarget
      if (pow.targetReset) {
        // Special behavior for (regtest and testnet):
...
class DeploymentState {
  ...
  /**
   * Test whether Difficulty Adjustment Algorithm is active.
   * @returns {Boolean}
   */
