...
class Chain extends AsyncEmitter {
 ...
 async cashRetarget(time, prev) {
   ...
   const last = this.getSuitableBlock(prev);
   assert(last);

   const heightFirst = height - 144;

   const first = this.getSuitableBlock(await this.getAncestor(heightFirst));
   assert(first);
