...
class Chain extends AsyncEmitter {
  ...
  setDeploymentState(state) {
    ...
    if (!this.state.hasSIGHASHFORKIDEnabled() && state.hasSIGHASHFORKIDEnabled())
  ...
  computeTarget(prev, first) {
    ...
    let work = prev.chainwork - first.chainwork;
    let actualTimespan = prev.time - first.time;
...
class DeploymentState {
  ...
  hasSIGHASHFORKIDEnabled() {
    return (this.flags & Script.flags.VERIFY_SIGHASH_FORKID || Script.flags.VERIFY_STRICTENC) !== 0;
  }
